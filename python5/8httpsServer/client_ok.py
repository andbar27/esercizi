...
def login():
    try:
        ...
        url = base_url + "/login"
        jsonDataRequest = {"username":user, "password":pw}
        response = requests.post(url, json=jsonDataRequest)

        if response.status_code == 200:
            print(response.json())
            return True, user, pw, response.json()["privilege"]
...
print(successLogin, " - ", user, " - ", pw, " - ", privilege)
...
while True:
    ...
    # Richiesta dati cittadino
        ...
        jsonDataRequest["datiCittadino"] = GetCodicefiscale()
        EseguiOperazione(2, api_url + "/" + jsonDataRequest["datiCittadino"]['codFiscale'],None)
